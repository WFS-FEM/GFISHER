...
depth.1min <- raster(paste0(dirname(dir.maps),"/depth/depth 1min 330x390.asc"))
depth.6min <- raster(paste0(dirname(dir.maps),"/depth/depth 6min 55x65.asc"))
depth.10min <- raster(paste0(dirname(dir.maps),"/depth/depth 10min 33x39.asc"))
...
png(filename=paste0(dir.output,"/Mapping footprint microgrids.png"),height=7,width=7,units='in',res=600)#,compression='lzw')
...
png(filename=paste0(dir.output,"/Mapping footprint plots.png"),height=7,width=7,units='in',res=600)#,compression='lzw')
...
png(filename=paste0(dir.output,"/Habitat pct area 1min.png"),height=7,width=7,units='in',res=600)#,compression='lzw')
...
writeRaster(
  habpct2.stack1,
  filename = file.path(dir.output, "reefhabitat_pct_1min"),
  bylayer = TRUE,
  suffix = names(habpct2.stack1),
  format = "ascii",
  overwrite = TRUE
)
...
png(filename=paste0(dir.output,"/Habitat pct area 6min.png"),height=7,width=7,units='in',res=600)#,compression='lzw')
par(mfcol=c(3,2),mar=c(3,3,3,5))
...
writeRaster(
  habpct2.stack6,
  filename = file.path(dir.output, "reefhabitat_pct_6min"),
  bylayer = TRUE,
  suffix = names(habpct2.stack6),
  format = "ascii",
  overwrite = TRUE
)
...
png(filename=paste0(dir.output,"/Habitat pct area 10min.png"),height=7,width=7,units='in',res=600)#,compression='lzw')
par(mfcol=c(3,2),mar=c(3,3,3,5))
...
writeRaster(
  habpct2.stack10,
  filename = file.path(dir.output, "reefhabitat_pct_10min"),
  bylayer = TRUE,
  suffix = names(habpct2.stack10),
  format = "ascii",
  overwrite = TRUE
)
...
#check sum
sum_raster <- calc(habpct2.stack1, fun = sum, na.rm = TRUE)
plot(sum_raster)
sum_raster
